...
<div class="max-w-2xl mx-auto bg-gray-800 p-6 rounded shadow text-white">
    <div class="flex justify-between items-center">
            <h1 class="text-2xl font-bold mb-4">Add Training</h1>
            <a href="{{ route('trainings.show') }}" 
            @click.prevent="loading = true; setTimeout(()=>{window.location.href='{{ route('trainings.show') }}'},500)"
            class="hover:bg-gray-400 hover:rounded-lg hover:py-1 hover:px-2 duration-300 transition-all"><i class="fas fa-arrow-left"></i></a>
        </div>
    <form action="{{ route('trainings.store') }}" @submit="loading = true;" method="POST">
        @csrf
        ...
        <!-- Training Date -->
        <label for="training_date" class="block my-2 font-medium">Training Date</label>
        <input type="datetime-local" name="training_date" id="" value="{{ old('training_date') }}" class="w-full border border-gray-600 bg-gray-700 text-white p-2 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">

        <!-- Location -->
        <div class="mt-4">
            <label for="location" class="block mb-2 font-medium">Location</label>
            <input type="text" name="location" id="location" value="{{ old('location') }}" placeholder="Enter training location"
                class="w-full border border-gray-600 bg-gray-700 text-white p-2 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
        </div>

        <!-- Focus -->
        <div class="mt-4">
            <label for="focus" class="block mb-2 font-medium">Focus</label>
            <input type="text" name="focus" id="focus" value="{{ old('focus') }}" placeholder="Enter focus"
                class="w-full border border-gray-600 bg-gray-700 text-white p-2 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
        </div>
