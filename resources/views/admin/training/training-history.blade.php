...
@section('title', 'Training History Page')
@section('content')
    ...
    <div class="max-w-6xl mx-auto bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg text-white">
        <h1 class="text-xl md:text-2xl font-bold mb-4">Training History Table</h1>
        <!-- Search Bar - Improved mobile layout -->
        <div class="flex flex-col sm:flex-row justify-between gap-4 mb-4">
            <form method="GET" action="{{ route('trainingHistory.show') }}" @submit="loading = true;" class="w-full sm:flex-1">
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-700">
            <table class="min-w-full divide-y divide-gray-700 text-sm text-left">
                <thead class="bg-gray-700 text-center text-gray-300">
                    <tr>
                        @foreach (['id', 'title', 'training_date', 'location', 'focus'] as $column)
                            <th class="px-3 py-2 md:px-4 md:py-2 font-medium whitespace-nowrap">
                                <!-- Sortable headers with better spacing -->
                                <a href="{{ route('trainingHistory.show', ['sort' => $column, 'direction' => $sortField === $column && $sortOrder === 'asc' ? 'desc' : 'asc']) }}"
                ...
                <tbody class="divide-y divide-gray-700 text-center">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-4 text-center text-gray-400">No training history found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
